...
def isotropic_ff(s,ion):
    fhd = open('pylib/3d_formfactors_j0.pck','rb')
    ...
    fhd = open('pylib/3d_formfactors_j2.pck','rb')
    form_factors_3d = pickle.load(fhd)
    fhd.close()
...
def TM_FormFactor(magQ, ion, gfactor=2):
    s = magQ/(4.*np.pi)
    j0, j2 = isotropic_ff(s,ion)
    j2factor = (2-gfactor)/gfactor
