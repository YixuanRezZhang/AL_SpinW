...
class lattice:
	...
	def inverseA(self,vect,norm=True):
		...
		if vect.size == 3:
			if norm:
				return np.linalg.norm(vect[0]*self.astar + vect[1]*self.bstar + vect[2]*self.cstar)
			else:
			...
			return np.multiply.outer(vect[:,:,0],self.astar) +\
					 np.multiply.outer(vect[:,:,1],self.bstar) +\
					  np.multiply.outer(vect[:,:,2],self.cstar)
		else:
			raise ValueError("vector must have three components") 
