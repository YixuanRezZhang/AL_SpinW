...
def BO_search_SW(Candidate_S, minhpar=1, option='FNGP', Q=0, exp_img_shape=exp_img_shape, batch=16):
    ...
    covar_module = ScaleKernel(MaternKernel(nu=2.5, ard_num_dims=dim, lengthscale_constraint=Interval(0.05, 4.0)))
    ...
    for count,index in enumerate(unnormalize(Candidate_S, ori_bounds)):
        ...
        fsample_base[tuple(map_index)] = fY[tuple(map_index)] + ratio*(res[count] + varhpar*var[count])
...
if restart is False or new_search is True:
    ...
    result = BO_search_SW_Turbo(fun_q, n_trust_regions=4, batch_size=4, max_evals=max_evals)
