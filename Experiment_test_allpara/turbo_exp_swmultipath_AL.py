...
def func(X, img_shape, energy_range, Q_paths, fit=True, csb=csb):
    ...
    J1, J2, J3, J4, J5, J6, J7, J8, DM1, dE = X
    #J1, J2, J3, J4, J5, J7, J8, dE = X
    
    Jfile = {'J1':J1, 'J2':J2, 'J3':J3, 'J4':J4, 'J5':J5, 'J6':J6, 'J7':J7, 'J8':J8}   # 8 parameters
...
## [J1, J2, J3, J4, J5, J6, J7, J8, DM1, dE]
lb = np.array([-4, -7, -4, -0.5, -0.5,   0,    0, -0.5,   0,  0])
ub = np.array([ 0,  0,  0,    0,    0, 0.5,  0.5,    0, 0.5,  6])
...
def candidate_selection(X, fX, lb=lb, ub=ub, max_size = None):
...
def BO_search_SW(Candidate_S, minhpar=1, option='FNGP', Q=0, exp_img_shape=exp_img_shape, batch=32):
    ...
    [_EX, _EY] = io.read_SW(kf_name=f'{Q}_SW_point.txt')
    ################################################ Bayesian Sampling ##################################################
    ...
    Y_train_diff = torch.tensor(_Y-new_fY, dtype=dtype, device=device).unsqueeze(-1)
    ...
    covar_module = ScaleKernel(MaternKernel(nu=2.5, ard_num_dims=dim, lengthscale_constraint=Interval(0.1, 4.0)))
    
    #model_G = FixedNoiseGP(X_ori, Y_train_diff, Yvar_diff, covar_module=covar_module)
    outcome_transform = Standardize(m=1)
    model_G = SingleTaskGP(X_ori, Y_train_diff, outcome_transform=outcome_transform) #, covar_module=covar_module)
    ...
    res = model_G_pred.mean
    var = torch.sqrt(model_G_pred.variance)

    print(Y_train_diff.mean(), Y_train_diff.std(), res.mean(), res.std())
    
    res = res.detach().cpu().numpy().squeeze()
    var = var.detach().cpu().numpy().squeeze()
    
    fsample_base = np.zeros(exp_img_shape[Q], dtype=float)
    fsample_resi = np.zeros(exp_img_shape[Q], dtype=float)
    fsample_resi_var = np.zeros(exp_img_shape[Q], dtype=float)

    exp_candindex = unnormalize(Candidate_S, ori_bounds).detach().cpu().numpy()
    sim_candindex = io.data_index_transformation(io.data_index_transformation(exp_candindex, 'expindex_2_expdata'), 'expdata_2_simuindex')
    
    for count,index in enumerate(exp_candindex):
        map_index = [int(i) for i in index]
        exp_map_index = [int(i) for i in sim_candindex[count]]
        ratio = _npfY[tuple(map_index)] / (_npfY[tuple(map_index)]+var[count])
        fsample_base[tuple(map_index)] = fY[tuple(exp_map_index)] + ratio*res[count]
        fsample_resi[tuple(map_index)] = res[count]
        fsample_resi_var[tuple(map_index)] = var[count]

    with open(f'{Q}_fsample.pickle', 'wb') as f:
        pickle.dump(fsample_base, f)
    ...
    Y_train_diff = torch.tensor(_EY-new_fY, dtype=dtype, device=device).unsqueeze(-1)
    ...
    outcome_transform = Standardize(m=1)
    model_G = SingleTaskGP(X_ori, Y_train_diff, outcome_transform=outcome_transform)
    mll_G = ExactMarginalLogLikelihood(model_G.likelihood, model_G)
    fit_gpytorch_model(mll_G)
    
    model_G_pred = model_G.posterior(Candidate_S)
    eres = model_G_pred.mean
    evar = torch.sqrt(model_G_pred.variance)
    
    eres = eres.detach().cpu().numpy().squeeze()
    evar = evar.detach().cpu().numpy().squeeze()
    
    efsample_resi = np.zeros(img_shape, dtype=float)
    efsample_resi_var = np.zeros(img_shape, dtype=float)
    
    for count,index in enumerate(unnormalize(Candidate_S, ori_bounds)):
        map_index = [int(i) for i in index]
        efsample_resi[tuple(map_index)] = eres[count]
        efsample_resi_var[tuple(map_index)] = evar[count]
        
    return {'X_best': Next_sample, 'Pred_residual': efsample_resi, 'Pred_resivar': efsample_resi_var}
...
def simple_KF(root_path, Q, predX_residual, predX_resivar, num_new):
    ...
    os.system(f'mv {Q}_fsample.pickle parameters/{str(int(folder))}/')
    ...
    print(f'trust_exp_noise_ratio: {trust_exp_noise_ratio}')
...
n = int(os.popen('cat '+root_path+'para_results.txt | wc -l').read())+1
...
while n < 100:
    ...
    for Q in range(len(Q_paths)):
        x_point = BO_search_SW(Candidate_S[Q][0], minhpar=1/np.sqrt(n), option='FNGP', Q=Q)
