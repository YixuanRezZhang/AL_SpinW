...
class TurboM(Turbo1):
    ...
    def optimize(self):
        ...
        while self.n_evals < self.max_evals:
            ...
            pre_X_next, idx_next = self._select_candidates(X_cand, y_cand)
            
            X_next = deepcopy(pre_X_next)
            ### Where to add the BWO position control
            kk = np.random.random(X_next.shape[:-1])*(1 - 0.5 * (Tstep + 1) / MaxTstep)
            #print(f'kk: {kk}')
            for i in range(self.n_trust_regions):
                if kk[i] > 0.5:
                    # Exploration phase - swimming
                    r1 = np.random.random()*self.length[i]/2
                    r2 = np.random.random()*self.length[i]/2
                    RJ = np.random.randint(self.n_trust_regions)
                    while RJ == i:
                        RJ = np.random.randint(self.n_trust_regions)
                    idx_i = np.where(idx_next == i)[0]
                    idx_rj = np.where(idx_next == RJ)[0]
                    assert len(idx_i)==len(idx_rj)
                    if self.dim <= self.n_trust_regions/5:
                        indices = np.arange(self.dim)
                        np.random.shuffle(indices)
                        
                        params = [indices[0], indices[1]]
                        X_next[idx_i][:, params[0]] = pre_X_next[idx_i][:, params[0]] + (pre_X_next[idx_rj][:, params[0]] - pre_X_next[idx_i][:, params[1]]) * (r1 + 1) * math.sin(r2 * 2 * math.pi)
                        X_next[idx_i][:, params[1]] = pre_X_next[idx_i][:, params[1]] + (pre_X_next[idx_rj][:, params[0]] - pre_X_next[idx_i][:, params[1]]) * (r1 + 1) * math.cos(r2 * 2 * math.pi)
                    else:
                        params = np.arange(self.dim)
                        np.random.shuffle(params)
                        for j in range(round(self.dim/2)):
                            X_next[idx_i][:, 2*j-1] = pre_X_next[idx_i][:, params[2*j-1]] + (pre_X_next[idx_rj][:, params[0]] - pre_X_next[idx_i][:, params[2*j-1]]) * (r1 + 1) * math.sin(r2 * 2 * math.pi)
                            X_next[idx_i][:, 2*j] = pre_X_next[idx_i][:, params[2*j]] + (pre_X_next[idx_rj][:, params[0]] - pre_X_next[idx_i][:, params[2*j]]) * (r1 + 1) * math.cos(r2 * 2 * math.pi)
                else:
                    # Development Phase - Predation
                    r3 = random.random()*self.length[i]/2
                    r4 = random.random()*self.length[i]/2
                    ...
                    if xposbest.ndim!=pre_X_next[idx_i].ndim:
                        xposbest = np.expand_dims(xposbest, axis=0)
                    #print(xposbest)
                    X_next[idx_i] = r3 * xposbest - r4 * pre_X_next[idx_i] + C1 * LevyFlight* (pre_X_next[idx_rj] - pre_X_next[idx_i])
